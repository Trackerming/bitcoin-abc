...
class AddressIndexTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-debug"], ["-debug", "-addressindex"],
                           ["-debug", "-addressindex"], ["-debug", "-addressindex"]]
    ...
    def run_test(self):
        ...
        multitxids = self.nodes[1].getaddresstxids({"addresses": [
                                                   "2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br", "mo9ncXisMeAoXwqcV5EWuyncbmCcQN4rVs"]})
        ...
        addressHash = bytes([99, 73, 164, 24, 252, 69, 120, 209,
                             10, 55, 43, 84, 180, 92, 40, 12, 200, 196, 56, 47])
        scriptPubKey = CScript([OP_HASH160, addressHash, OP_EQUAL])
        unspent = self.nodes[0].listunspent()
        tx = CTransaction()
        tx.vin = [
            CTxIn(COutPoint(int(unspent[0]["txid"], 16), unspent[0]["vout"]))]
        tx.vout = [CTxOut(4990000000, scriptPubKey), CTxOut(9100000, scriptPubKey)]
        tx.rehash()
        ...
        balance0 = self.nodes[1].getaddressbalance(
            "2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br")
        assert_equal(balance0["balance"], 45 * 100000000 + 4990000000+9100000)

        # Check that balances are correct after spending
        print("Testing balances after spending...")
        privkey2 = "cSdkPxkAjA4HDr5VHgsebAPDEh9Gyub4HK8UJr2DFGGqKKy4K5sG"
        address2 = "mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW"
        addressHash2 = bytes([11, 47, 10, 12, 49, 191, 224, 64,
                              107, 12, 204, 19, 129, 253, 190, 49, 25, 70, 218, 220])
        ...
        tx.vout = [CTxOut(change_amount, scriptPubKey2),
        ...
        deltas = self.nodes[1].getaddressdeltas(
            {"addresses": [address2], "start": 1, "end": 200})
        balance3 = 0
        for delta in deltas:
        ...
        deltas = self.nodes[1].getaddressdeltas(
            {"addresses": [address2], "start": 113, "end": 113})
        ...
        self.sync_all()
        log_hash = self.nodes[2].getbestblockhash()
        print(log_hash)
        best_hash = self.nodes[0].getbestblockhash()
        balance4 = self.nodes[0].getaddressbalance(address2)
        ...
        addressHash3 = bytes([170, 152, 114, 181, 187, 205, 181,
                              17, 216, 158, 14, 17, 170, 39, 218, 115, 253, 44, 63, 80])
        ...
        tx.rehash()
        signed_tx = self.nodes[2].signrawtransactionwithwallet(
            binascii.hexlify(tx.serialize()).decode("utf-8"))
        ...
        tx2.rehash()
        signed_tx2 = self.nodes[2].signrawtransactionwithwallet(
        ...
        signed_tx3 = self.nodes[2].signrawtransactionwithwallet(
            binascii.hexlify(tx.serialize()).decode("utf-8"))
        ...
        address1hash = bytes([193, 146, 191, 247, 81, 175, 142, 254,
                              193, 81, 53, 212, 43, 254, 237, 249, 26, 111, 62, 52])
        ...
        utxos_with_info = self.nodes[1].getaddressutxos(
            {"addresses": [address2], "chainInfo": True})
