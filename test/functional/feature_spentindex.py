...
class SpentIndexTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-debug"], ["-debug", "-spentindex"],
                           ["-debug", "-spentindex"], ["-debug", "-spentindex", "-txindex"]]
    ...
    def run_test(self):
        ...
        addressHash = bytes([11, 47, 10, 12, 49, 191, 224, 64,
                             107, 12, 204, 19, 129, 253, 190, 49, 25, 70, 218, 220])
        scriptPubKey = CScript(
            [OP_DUP, OP_HASH160, addressHash, OP_EQUALVERIFY, OP_CHECKSIG])
        ...
        info = self.nodes[1].getspentinfo(
            {"txid": unspent[0]["txid"], "index": unspent[0]["vout"]})
        ...
        assert_equal(float(txVerbose2["vin"][0]["value"]),
                     (amount + feeSatoshis) / 100000000)
        ...
        assert_equal(float(txVerbose3["vin"][0]["value"]),
                     (amount + feeSatoshis) / 100000000)
        ...
        assert_equal(float(txVerbose4["vin"][0]["value"]),
                     (amount + feeSatoshis) / 100000000)
        ...
        assert_equal(block["deltas"][1]["outputs"][0]
                     ["address"], "mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW")
