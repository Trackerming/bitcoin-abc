...
void CTxMemPool::addAddressIndex(const CTxMemPoolEntry &entry,
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (prevout.scriptPubKey.IsPayToScriptHash()) {
            std::vector<uint8_t> hashBytes(prevout.scriptPubKey.begin() + 2,
                                           prevout.scriptPubKey.begin() + 22);
            CMempoolAddressDeltaKey key(2, uint160(hashBytes), txhash, j, 1);
            CMempoolAddressDelta delta(
                entry.GetTime().count() , prevout.nValue / SATOSHI * -1,
                input.prevout.GetTxId(), input.prevout.GetN());
            mapAddress.insert(std::make_pair(key, delta));
            inserted.push_back(key);
        } else if (prevout.scriptPubKey.IsPayToPublicKeyHash()) {
            std::vector<uint8_t> hashBytes(prevout.scriptPubKey.begin() + 3,
    ...
    for (unsigned int k = 0; k < tx.vout.size(); k++) {
        ...
        if (out.scriptPubKey.IsPayToScriptHash()) {
            ...
            mapAddress.insert(std::make_pair(
                key,
                CMempoolAddressDelta(entry.GetTime().count(), out.nValue / SATOSHI)));
            inserted.push_back(key);
...
bool CTxMemPool::getAddressIndex(
    ...
    for (std::vector<std::pair<uint160, int>>::iterator it = addresses.begin();
         it != addresses.end(); it++) {
        addressDeltaMap::iterator ait = mapAddress.lower_bound(
            CMempoolAddressDeltaKey((*it).second, (*it).first));
        while (ait != mapAddress.end() &&
               (*ait).first.addressBytes == (*it).first &&
...
bool CTxMemPool::removeAddressIndex(const uint256 txhash) {
    ...
    if (it != mapAddressInserted.end()) {
        ...
        for (std::vector<CMempoolAddressDeltaKey>::iterator mit = keys.begin();
             mit != keys.end(); mit++) {
...
void CTxMemPool::addSpentIndex(const CTxMemPoolEntry &entry,
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (prevout.scriptPubKey.IsPayToScriptHash()) {
            addressHash = uint160(
                std::vector<uint8_t>(prevout.scriptPubKey.begin() + 2,
                                     prevout.scriptPubKey.begin() + 22));
            addressType = 2;
        } else if (prevout.scriptPubKey.IsPayToPublicKeyHash()) {
            addressHash = uint160(
                std::vector<uint8_t>(prevout.scriptPubKey.begin() + 3,
                                     prevout.scriptPubKey.begin() + 23));
            addressType = 1;
        } else {
            addressHash.SetNull();
        ...
        CSpentIndexValue value = CSpentIndexValue(
            txhash, j, -1, prevout.nValue / SATOSHI, addressType, addressHash);
...
bool CTxMemPool::removeSpentIndex(const uint256 txhash) {
    ...
    if (it != mapSpentInserted.end()) {
        ...
        for (std::vector<CSpentIndexKey>::iterator mit = keys.begin();
             mit != keys.end(); mit++) {
            mapSpent.erase(*mit);
        }
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason) {
    ...
    mapTx.erase(it);
    nTransactionsUpdated++;
    removeAddressIndex(it->GetTx().GetId());
    removeSpentIndex(it->GetTx().GetId());
}
