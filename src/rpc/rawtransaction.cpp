...
void TxToJSONExpanded(const Config &config, const CTransaction &tx,
                      const uint256 hashBlock, UniValue &entry, int nHeight = 0,
                      int nConfirmations = 0, int nBlockTime = 0) {
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (tx.IsCoinBase())
            in.pushKV("coinbase", HexStr(std::vector<uint8_t>(txin.scriptSig.begin(), txin.scriptSig.end())));
        else {
            in.pushKV("txid", txin.prevout.GetTxId().GetHex());
            in.pushKV("vout", (int64_t)txin.prevout.GetN());
            UniValue o(UniValue::VOBJ);
            o.pushKV("asm", ScriptToAsmStr(txin.scriptSig, true));
            o.pushKV("hex", HexStr(std::vector<uint8_t>(txin.scriptSig.begin(), txin.scriptSig.end())));
            in.pushKV("scriptSig", o);

            // Add address and value info if spentindex enabled
            CSpentIndexValue spentInfo;
            CSpentIndexKey spentKey(txin.prevout.GetTxId(),
                                    txin.prevout.GetN());
            if (GetSpentIndex(spentKey, spentInfo)) {
                in.pushKV("value", ValueFromCAmount(spentInfo.satoshis));
                in.pushKV("valueSat", spentInfo.satoshis);
                if (spentInfo.addressType == 1) {
                    in.pushKV("address", EncodeLegacyAddr(PKHash(spentInfo.addressHash),
                                               config.GetChainParams()));
                } else if (spentInfo.addressType == 2) {
                    in.pushKV("address", EncodeLegacyAddr(ScriptHash(spentInfo.addressHash),
                                               config.GetChainParams()));
                }
            }
        }
...
static UniValue sendrawtransaction(const Config &config,
    ...
    const CFeeRate max_raw_tx_fee_rate =
        request.params[1].isNull() || request.params[1].isBool()
